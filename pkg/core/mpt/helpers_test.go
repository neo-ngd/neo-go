...
func TestGetChildrenPaths(t *testing.T) {
	h1 := NewHashNode(common.Hash{1, 2, 3})
	h2 := NewHashNode(common.Hash{4, 5, 6})
	h3 := NewHashNode(common.Hash{7, 8, 9})
	...
	testCases := map[string]struct {
		...
		"Hash":                         {h1, nil},
		"Leaf":                         {l, nil},
		"Extension with next Hash":     {ext1, map[common.Hash][][]byte{h1.Hash(): {ext1.key}}},
		"Extension with next non-Hash": {ext2, map[common.Hash][][]byte{}},
		"Branch": {branch, map[common.Hash][][]byte{
			h1.Hash(): {{0x03}, {0x06}},
			h2.Hash(): {{}},
			h3.Hash(): {{0x07}},
		}},
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if testCase.expected != nil {
				expectedWithPrefix := make(map[common.Hash][][]byte, len(testCase.expected))
